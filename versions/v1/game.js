(() => {
  ...
  const passives=[
    ...
    {id:'luck',name:'金色票根',icon:'☘',r:'rare',desc:'提高高稀有度选项概率，并提升高稀有攻击的抽取权重',max:4,apply:g=>{g.p.luck++;g.p.rareBias+=.25}},
  ...
  const passiveMap=Object.fromEntries(passives.map(p=>[p.id,p]));
  const rarityDrawFactor={common:0,rare:.5,epic:.75,legend:1};
  ...
  function renderPauseSlots(){const p=game.p,hand=game.lastHand;$('#pauseDrawMeta').textContent=`${p.draws} SLOTS`;$('#pauseSlots').innerHTML=Array.from({length:p.draws},(_,i)=>{const item=hand[i],a=item?.drawn,rank=a?rarity[a.r]:null;return `<div class="pause-slot" style="--slot-color:${rank?.color||'#334155'}"><div class="slot-number">SLOT ${String(i+1).padStart(2,'0')}</div><div class="slot-icon">${a?.icon||'◇'}</div><b style="color:${rank?.color||'#64748b'}">${a?.name||'等待抽取'}</b></div>`}).join('');const stats=[['生命',`${Math.ceil(p.hp)} / ${p.maxHp}`],['伤害倍率',`${Math.round(p.damage*100)}%`],['移动速度',Math.round(p.speed)],['攻击间隔',`${p.interval.toFixed(2)}s`],['护甲减伤',`${Math.round(p.armor*100)}%`],['暴击率',`${Math.round(p.crit*100)}%`],['拾取范围',Math.round(p.magnet)],['重复施放',`${Math.round(p.repeat*100)}%`],['范围倍率',`${Math.round(p.area*100)}%`],['传说权重',`+${Math.round(p.rareBias*100)}%`]];$('#pauseStats').innerHTML=stats.map(([k,v])=>`<div class="pause-stat"><span>${k}</span><b>${v}</b></div>`).join('');const owned=Object.entries(game.passiveLevels);$('#pausePassives').innerHTML=owned.length?owned.map(([id,level])=>{const item=passiveMap[id],rank=rarity[item.r];return `<div class="passive-chip"><span>${item.icon}</span><b style="color:${rank.color}">${item.name}</b><em>LV.${level}</em></div>`}).join(''):'<div class="passive-empty">尚未选择被动技能</div>'}
